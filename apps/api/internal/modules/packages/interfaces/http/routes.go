// Package http contiene los adaptadores HTTP del modulo packages.
//
// Los handlers traducen request/response al usecase correspondiente y
// emiten errores RFC 7807 via apperrors. NO contienen logica de negocio.
package http

import (
	"net/http"

	"github.com/go-chi/chi/v5"
)

// MountOption configura el montaje del modulo. Permite inyectar guards
// (RBAC) sin que el modulo importe paquetes de autorizacion.
type MountOption func(*mountConfig)

type mountConfig struct {
	guard func(ns string) func(http.Handler) http.Handler
}

// WithGuard permite que el orquestador (cmd/api) pase un constructor de
// middleware RBAC que el modulo aplica por endpoint.
func WithGuard(g func(ns string) func(http.Handler) http.Handler) MountOption {
	return func(c *mountConfig) { c.guard = g }
}

// Mount monta los endpoints del modulo packages en r.
//
// Endpoints:
//
//	POST   /packages                            package.create
//	GET    /packages?unit_id=&status=           package.read
//	GET    /packages/{id}                       package.read
//	POST   /packages/{id}/deliver-by-qr         package.deliver
//	POST   /packages/{id}/deliver-manual        package.deliver
//	POST   /packages/{id}/return                package.deliver
//	GET    /package-categories                  package.read
//
// El modulo NO conoce el modulo authorization: si se desea gating RBAC,
// se inyecta via WithGuard. Si no, el guard es no-op.
func Mount(r chi.Router, deps Dependencies, opts ...MountOption) {
	cfg := &mountConfig{}
	for _, o := range opts {
		o(cfg)
	}

	h := newHandlers(deps)

	gate := func(ns string) func(http.Handler) http.Handler {
		if cfg.guard == nil {
			return func(next http.Handler) http.Handler { return next }
		}
		return cfg.guard(ns)
	}

	r.Route("/packages", func(pr chi.Router) {
		pr.With(gate("package.create")).Post("/", h.createPackage)
		pr.With(gate("package.read")).Get("/", h.listPackages)
		pr.With(gate("package.read")).Get("/{id}", h.getPackage)
		pr.With(gate("package.deliver")).Post("/{id}/deliver-by-qr", h.deliverByQR)
		pr.With(gate("package.deliver")).Post("/{id}/deliver-manual", h.deliverManual)
		pr.With(gate("package.deliver")).Post("/{id}/return", h.returnPackage)
	})

	r.Route("/package-categories", func(cr chi.Router) {
		cr.With(gate("package.read")).Get("/", h.listCategories)
	})
}
